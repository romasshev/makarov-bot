...
async def sendall(message: types.Message):
    if message.chat.type == 'private':
        if message.from_user.id == 340207093:
            text = message.text[9:]
            users = db.get_users()
            for row in users:
                try:
...
async def countusers(message: types.Message):
    if message.chat.type == 'private':
        if message.from_user.id == 340207093:
            await bot.send_message(message.from_user.id,(f'В базе данных {db.count_users()} активных уникальных пользователей'))
        else:
            await bot.send_message(message.from_user.id,("Вы не можете использовать эту команду"))
...
async def process_callback_button1(call: types.CallbackQuery):
    await call.answer()
    await call.message.edit_text(text='<b>Базовая:</b>\n\nДлительность: 1 час 30 минут \n\nЗнакомство, разбор причины, проработка, плавный выход + получение клиентом инструментов для дальнейшей самостоятельной работы с проблемой \n\n<b>Стоимость - 5000р</b>', reply_markup=inline_kb_full2)
